...
comp.time = rep(NA, length(lmark)); j = 0
...
for (l in lmark){
  ...
  for(seed in seeds.RCV){
    ...
    for(k.fold in 1:n.fold){
      ...
      pve = 0.9 # proportion of variance explained
      ...
      res = try(for(p in 1:Q){
        tmp.ufpca = uPACE(multivar.train[,,p], argvals, nbasis=3, npc = npc, pve = pve)
      ...
      if (class(res)!='try-error'){
        ...
        surv.train.temp = cbind(surv.train[,c('time.to.event','event',fixed.cov.names)], rho.train) #don't take the id
        CoxFit = coxph(Surv(time.to.event, event) ~., data = surv.train.temp, model = TRUE, x = TRUE)
        ...
        surv.test.rho = cbind(surv.test[,c('time.to.event','event',fixed.cov.names)],rho.test) #don't take the id column
        ...
        for(dt in times){
          ith = ith + 1
          surv.prob[[ith]] = cond.prob(CoxFit, surv.test.rho, l, dt)  # risk prediction
        }  
