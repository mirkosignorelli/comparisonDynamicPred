...
cond.prob = function(model, newdata, Tstart, Tpred){
  ...
  res = risk.Tpred/risk.Tstart
  if(sum(is.na(res)) != 0){
    print(paste0('WARNING: For ',sum(is.na(res)),' subjects [',which(is.na(res)),'] at time l+dt=',Tpred,' function survfit calculated 0 survival probability at landmark time. The conditional probability resulting NaN is set to 0.'))
